...
void DataHandler::read_feature_vector(const std::string &path)
{
    ...
    { //header[1]表示文件中图片的个数
        ...
        std::vector<uint8_t> vec(
            image_buf,
            image_buf + nread);
        data.set_feature_vector(std::move(vec));
        data_array.emplace_back(std::move(data));
    }
    fclose(f);
...
void DataHandler::read_feature_labels(const std::string &path)
{
    ...
    {
        data_array[i].set_label(labels[i]);
    }
...
const std::vector<Data> &DataHandler::get_training_data()
{
    return training_data;
}
const std::vector<Data> &DataHandler::get_test_data()
{
    return test_data;
}

const std::vector<Data> &DataHandler::get_validataion_data()
{
